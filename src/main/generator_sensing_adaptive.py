...
def Mask(x):
    ...
    idx_value = tf.tile(idx_value, [1, height, width, 1])
...
class Sampling_plus_init_reconstruction_network(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.sensing_importance = layers.Conv2D(30, 32, strides=[32, 32], use_bias=False, padding='valid',
                                                activation=None)
        ...
        self.sensing_conv = layers.Conv2D(140, 32, strides=[32,32],use_bias=False, padding='valid', activation=None)
        self.conv4 = layers.Conv2D(256, 1, padding='same', activation='relu')
        self.conv5 = layers.Conv2D(256, 1, padding='same', activation='relu')
        self.conv6 = layers.Conv2D(256, 1, padding='same', activation='relu')
        self.re_conv = layers.Conv2D(32*32*3, 1, strides=[1, 1], use_bias=False, padding='valid', activation=None)
    def call(self, inputs, **kwargs):
        # inputs  [batch,224,224,3]

        # x_sliced = tf.image.extract_patches(images=inputs,
        #                                     sizes=[1, 32, 32, 1],
        #                                     strides=[1, 32, 32, 1],
        #                                     rates=[1, 1, 1, 1],
        #                                     padding='VALID')
        # x_sliced_reshaped = tf.reshape(x_sliced, [-1, 7, 7, 1, 32*32*3])
        # sampling_weight=tf.tile(self.sampling_weight,[1,7,7,1,1])
        # x_sampled = tf.matmul(x_sliced_reshaped,sampling_weight) #[batch,7,7,1,103]
        # reconstruction_weight = tf.tile(self.reconstruction_weight, [1, 7, 7, 1, 1])
        # x_sampled = tf.matmul(x_sampled, reconstruction_weight) #[batch,7,7,1,32*32*3]
        # x_sampled_reshaped=tf.reshape(x_sampled, [-1, 7, 7, 32, 32, 3])
        #
        # x_sliced_transpose = tf.transpose(x_sampled_reshaped, [0, 1, 3, 2, 4, 5])
        #
        # x_sliced_re = tf.reshape(x_sliced_transpose, [-1, 224, 224, 3])
        ...
        rate = self.conv3(first_x_feature)

        # first_x_feature_transfered = self.conv4(tf.stop_gradient(first_x))
        # first_x_feature_transfered = self.conv5(first_x_feature_transfered)
        # first_x_feature_transfered = self.conv6(first_x_feature_transfered)

        bit = Quantizer(rate)
        m = Mask(bit)

        m_mean, m_var = tf.nn.moments(tf.reduce_sum(m, axis=-1), axes=[0, 1, 2])
...
class Deep_reconstruction_network(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.init_conv = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.res1_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.res1_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.res2_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.res2_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.res3_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.res3_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.res4_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.res4_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.res5_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.res5_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.out_conv = layers.Conv2D(3, 3, padding='same', activation=None)
