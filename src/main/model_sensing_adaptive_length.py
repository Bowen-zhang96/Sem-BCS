...
from main.generator_with_sensing_adaptive import Generator
...
class Model:
    ...
    def train(self):
        ...
        for epoch in range(start, self.config.EPOCHS + 1):
            ...
            # weight = self.generator._get_sampling_weight()
            # weight = weight.numpy()
    ...
    def _train_step(self, images, kp2d, kp3d, has3d, theta):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            rate_loss = rate* self.config.Rate_Lambda * self.config.GENERATOR_RATE_LOSS_WEIGHT
            ...
            generator_loss = tf.reduce_sum([kp2d_loss, gen_disc_loss, rate_loss])
            if self.config.USE_3D:
        ...
        vars_others=[]
        grads_others=[]
        for var, grade in zip(self.generator.trainable_variables, generator_grads):
            if 'sampling' in var.name:
                vars_rate.append(var)
                grads_rate.append(grade)
            else:
                vars_others.append(var)
                grads_others.append(grade)
        if vars_rate==[]:
            print('error////////////')
        self.generator_opt.apply_gradients(zip(grads_others, vars_others))
        self.generator_rate_opt.apply_gradients(zip(grads_rate, vars_rate))
    ...
    def _log_train(self, epoch):
        ...
        template = 'kp2d_loss: {}, rate_loss: {}, kp3d_loss: {}, train_bit_length: {}, train_bit_length_var: {}'
        print(template.format(self.kp2d_loss_log.result(), self.rate_loss_log.result(),
                              self.kp3d_loss_log.result(), self.bit_train_log.result(), self.bit_train_var_log.result()))
    ...
    def _val_step(self, images, kp2d, kp3d, has3d):
        ...
        result, rate, m_mean, m_var = self.generator(images, training=False)
    ...
    def _test_step(self, image, kp2d, kp3d, has3d, return_kps=False):
        ...
        result, rate, m_mean, m_var  = self.generator(image, training=False)
        # only use last computed theta (from accumulated iterative feedback loop)
        _, kp2d_pred, kp3d_pred, _, _, _ = result[-1]
        vis = kp2d[:, :, 2]
        kp2d_norm = tf.norm(kp2d_pred[:, :self.config.NUM_KP2D, :] - kp2d[:, :, :2], axis=2) * vis
        ...
        if return_kps:
            return kp2d_mpjpe,  kp2d_pred,  kp2d, kp3d_mpjpe, kp3d_mpjpe_aligned, kp3d_pred,  kp3d, m_mean

        return kp2d_mpjpe,  None, None, kp3d_mpjpe, kp3d_mpjpe_aligned, None, None, m_mean
