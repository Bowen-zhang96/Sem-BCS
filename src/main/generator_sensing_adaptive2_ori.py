...
class Sampling_plus_init_reconstruction_network(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.conv5 = layers.Conv2D(200, 1, padding='same', activation='sigmoid')
        ...
        self.conv7 = layers.Conv2D(220 * 220, 1, padding='same', activation=None)
    ...
    def call(self, inputs, **kwargs):
        ...
        rate = self.conv4(first_x_feature)
        rate = self.conv5(rate)
        ...
        # x = x * m
        p = tf.random.uniform(shape=tf.shape(tf.concat([first_x, x], axis=-1)))

        def adaptive_function(x,m):
            ...
            return concated_sensing

        def fixed_function(x):

            concated_sensing_o = tf.concat([first_x, x], axis=-1)

            return concated_sensing_o

        concated_sensing = tf.where(p > -1., adaptive_function(x,m), fixed_function(x))
...
class Deep_reconstruction_network(tf.keras.Model):
    ...
    def __init__(self):
        super(Deep_reconstruction_network, self).__init__(name='CSNet_re')

        self.CSNet_init_conv = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_res1_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.CSNet_res1_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_res2_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.CSNet_res2_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_res3_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.CSNet_res3_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_res4_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.CSNet_res4_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_res5_1 = layers.Conv2D(64, 3, padding='same', activation='relu')
        self.CSNet_res5_2 = layers.Conv2D(64, 3, padding='same', activation='relu')

        self.CSNet_out_conv = layers.Conv2D(3, 3, padding='same', activation=None)


    def call(self, inputs, **kwargs):
        x = self.CSNet_init_conv(inputs)

        x_1 = self.CSNet_res1_1(x)
        x = x + x_1
        x = self.CSNet_res1_2(x)

        x_1 = self.CSNet_res2_1(x)
        x = x + x_1
        x = self.CSNet_res2_2(x)

        x_1 = self.CSNet_res3_1(x)
        x = x + x_1
        x = self.CSNet_res3_2(x)

        x_1 = self.CSNet_res4_1(x)
        x = x + x_1
        x = self.CSNet_res4_2(x)

        x_1 = self.CSNet_res5_1(x)
        x = x + x_1
        x = self.CSNet_res5_2(x)

        x = self.CSNet_out_conv(x)
        x = x + inputs
...
class Generator(tf.keras.Model):
    ...
    def call(self, inputs, **kwargs):
       ...
       #  for i in range(self.config.ITERATIONS):
       #      theta = thetas[i, :]
       #      outputs.append(self._compute_output(theta, **kwargs)), m, m=
