...
from main.generator_sensing_adaptive import Generator
...
class Model:
    ...
    def test(self, return_kps=True):
        ...
        for image_data in tqdm(ds_test, total=total, position=0, desc='testing'):
            ...
            psnr_, bit_ = self._test_step(image, kp2d, kp3d, has_3d, return_kps=return_kps)
        ...
        result_dict = {"psnr": tf.reduce_mean(tf.stack(psnr)),"bit": tf.reduce_mean(tf.stack(bit))}
